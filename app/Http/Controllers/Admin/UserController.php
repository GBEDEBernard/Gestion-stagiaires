...
class UserController extends Controller
{
    ...
    protected function validateUserPayload(Request $request, ?User $user = null): array
    {
        ...
        $rules = [
            ...
            'site_id' => ['nullable', 'exists:sites,id', $isEmploye ? 'required' : 'nullable'],
        ];

        // Validation personnalisée : vérifier que le domaine appartient au site choisi
        $rules['domaine_id'][] = function ($attribute, $value, $fail) use ($request) {
            if (!$request->filled('site_id') || !$value) {
                return;
            }
            $site = Site::find($request->input('site_id'));
            $domaine = Domaine::find($value);
            if ($site && $domaine && !$site->domaines()->where('domaines.id', $value)->exists()) {
                $fail('Le domaine sélectionné n\'appartient pas au site choisi.');
            }
        };
