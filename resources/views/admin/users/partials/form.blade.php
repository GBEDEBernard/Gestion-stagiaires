        ...
        <section class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="nom" id="nom" value="{{ $nomValue }}" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Ex: Dupont">
                ...
                <div>
                    ...
                    <input type="text" name="prenom" id="prenom" value="{{ $prenomValue }}" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Ex: Jean">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $user?->email) }}" required class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="exemple@email.com">
                ...
                <div>
                    ...
                    <input type="password" name="password" id="password" @unless($user) required @endunless class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Définis le mot de passe initial">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="password" name="password_confirmation" id="password_confirmation" @unless($user) required @endunless class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Confirme le mot de passe temporaire">
                </div>
            </div>
        </section>
        ...
        <section class="space-y-5 border-t border-gray-100 dark:border-gray-700 pt-6">
            ...
            <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4">
                ...
                @unless($isCreate)
                                ...
                                <div class="space-y-2">
                                    @foreach($groupPermissions as $permission)
                                        <label class="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-300">
                                            <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-emerald-500 permission-checkbox" {{ in_array($permission->name, $selectedPermissionNames) ? 'checked' : '' }}>
                    ...
                    @error('permissions')<p class="mt-3 text-sm text-red-500">{{ $message }}</p>@enderror
                @endunless
            </div>
        </section>

        {{-- SECTION DOMAINE DE TRAVAIL (employé) --}}
        <section class="space-y-5 border-t border-gray-100 dark:border-gray-700 pt-6 {{ $showDomaineSection ? '' : 'hidden' }}" data-domaine-section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="site_id" id="site_id" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white" required>
                    ...
                    @error('site_id')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
                ...
                <div>
                    ...
                    <select name="domaine_id" id="domaine_id" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white" required disabled>
            ...
            {{-- NOUVEAUX CHAMPS : Matricule et Poste --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                <div>
                    <label for="matricule" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Matricule</label>
                    <input type="text" name="matricule" id="matricule" value="{{ old('matricule') }}" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Laisser vide pour générer automatiquement">
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Un matricule sera généré automatiquement si le champ est vide.</p>
                    @error('matricule')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="poste" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Poste</label>
                    <input type="text" name="poste" id="poste" value="{{ old('poste') }}" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Ex: Comptable, Développeur...">
                    @error('poste')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
            </div>
        </section>

        {{-- SECTION FICHE ÉTUDIANT --}}
        <section class="space-y-5 border-t border-gray-100 dark:border-gray-700 pt-6 {{ $shouldShowEtudiantBlock ? '' : 'hidden' }}" data-etudiant-fields>
            <div>
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Fiche étudiant</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Ce bloc apparaît quand le rôle <span class="font-semibold">étudiant</span> est choisi.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <select name="etudiant_genre" id="etudiant_genre" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white">
                    ...
                    @error('etudiant_genre')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="etudiant_telephone" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Téléphone</label>
                    <input type="text" name="etudiant_telephone" id="etudiant_telephone" value="{{ old('etudiant_telephone', $linkedEtudiant?->telephone) }}" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Ex: +229 01 00 00 00 00">
                    @error('etudiant_telephone')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="etudiant_ecole" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">École</label>
                    <input type="text" name="etudiant_ecole" id="etudiant_ecole" value="{{ old('etudiant_ecole', $linkedEtudiant?->ecole) }}" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition text-gray-900 dark:text-white placeholder-gray-400" placeholder="Ex: Université d'Abomey-Calavi">
                    @error('etudiant_ecole')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
            </div>
        </section>

        <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-700">
            <a href="{{ route('admin.users.index') }}" class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition font-medium">Annuler</a>
            <button type="submit" class="px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-700 text-white rounded-xl hover:from-green-700 hover:to-emerald-800 transition font-medium shadow-lg shadow-green-600/20 flex items-center gap-2">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const defaultPermissions = () => {
            ...
            selectedRole().forEach(roleName => {
                (roleMap[roleName] || []).forEach(p => permissions.add(p));
            });
            return permissions;
        };

        const syncEtudiantVisibility = () => {
            etudiantFields.classList.toggle('hidden', !selectedRole().includes('etudiant'));
        };
        const syncDomaineVisibility = () => {
            domaineSection?.classList.toggle('hidden', !selectedRole().includes('employe'));
        };

        const applyTypeDefaults = () => {
            hiddenRoleInput.value = typeSelect.value || '';
            if (permissionsOverriddenInput.value !== '1') {
                const defaults = defaultPermissions();
                permissionCheckboxes.forEach(cb => { cb.checked = defaults.has(cb.value); });
            }
        ...
        permissionCheckboxes.forEach(cb => cb.addEventListener('change', () => {
            ...
            const defaults = defaultPermissions();
            permissionCheckboxes.forEach(c => c.dataset.manual = String(c.checked !== defaults.has(c.value)));
        }));

        applyTypeDefaults();
        // Chargement dynamique domaines par site
        ...
        if (siteSelect && domaineSelect) {
            siteSelect.addEventListener('change', async function() {
                ...
                try {
                    const resp = await fetch(`/api/sites/${siteId}/domaines`);
                    const domaines = await resp.json();
            ...
            if (siteSelect.value) siteSelect.dispatchEvent(new Event('change'));
        }
    });
</script>