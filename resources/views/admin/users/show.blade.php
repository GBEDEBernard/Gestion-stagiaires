<x-app-layout>
    @php
        ...
        $nom      = $personnel?->nom    ?? '';
        $initials = mb_strtoupper(mb_substr($prenom, 0, 1) . mb_substr($nom, 0, 1)) ?: 'US';
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">Profil utilisateur</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    <a href="{{ route('admin.users.index') }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition">Utilisateurs</a>
        ...
        <div class="flex flex-wrap gap-2.5">
            ...
            <button type="button"
                ...
                class="open-toggle-modal inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition shadow-sm
                    {{ $user->status === 'actif'
            ...
            <form action="{{ encrypted_route('admin.users.destroy', $user) }}" method="POST"
                  onsubmit="return confirm('Supprimer définitivement {{ $displayName }} ? Cette action est irréversible.')">
                @csrf @method('DELETE')
                <button type="submit"
                    class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 font-medium hover:bg-red-200 dark:hover:bg-red-900/50 transition shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="relative h-28 bg-gradient-to-r from-blue-500 to-indigo-600 dark:from-blue-700 dark:to-indigo-800">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-xl p-4 border border-amber-100 dark:border-amber-800/30">
                            ...
                            <p class="text-gray-800 dark:text-gray-200 font-semibold text-sm">Inscrit : {{ $dateInscriptionFormatted }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Pointage : {{ $dateDebutPointageFormatted }}</p>
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4 text-sm">
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-gray-500 dark:text-gray-400">Rôle(s)</span>
                        <div class="flex flex-wrap gap-1 justify-end">
                            @foreach($user->getRoleNames() as $role)
                            <span class="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-xs px-2 py-0.5 rounded">{{ $role }}</span>
                            @endforeach
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            cancelBtn?.addEventListener('click', () => { modal.classList.add('hidden'); actionUrl = null; });
            modal.addEventListener('click', e => { if (e.target === modal) { modal.classList.add('hidden'); actionUrl = null; } });
