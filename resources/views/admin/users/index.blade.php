    ...
    <form method="GET" action="{{ route('admin.users.index') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            ...
            <div class="sm:col-span-2 xl:col-span-2">
                ...
                <div class="relative">
                    ...
                    <input type="search" name="search" value="{{ request('search') }}" placeholder="Nom ou email..."
                           class="w-full pl-9 pr-4 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500 focus:border-transparent">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1.5">Statut</label>
                <select name="status" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500">
            ...
            <div>
                ...
                <select name="verified" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500">
                    <option value="">Tous</option>
            ...
            <div>
                ...
                <select name="role" class="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500">
                    <option value="">Tous</option>
            ...
            <div class="flex items-end gap-2">
                <button type="submit"
                        class="flex-1 inline-flex justify-center items-center gap-1.5 rounded-xl bg-sky-600 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-700 transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2a1 1 0 01-.293.707L13 13.414V19a1 1 0 01-.553.894l-4 2A1 1 0 017 21v-7.586L3.293 6.707A1 1 0 013 6V4z"/></svg>
                    Filtrer
                </button>
                <a href="{{ route('admin.users.index') }}"
                   class="flex-1 inline-flex justify-center rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    Reset
                </a>
            </div>
        </div>
        ...
        <div class="mt-3 flex items-center justify-between">
            <p class="text-xs text-gray-400">{{ $users->total() }} utilisateur(s) trouvé(s)</p>
        </div>
    </form>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                        ...
                        {{-- Permissions masquées sur petits écrans --}}
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        ...
                        <td class="px-1 py-3.5">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="shrink-0 w-9 h-9 rounded-full overflow-hidden
                                    {{ $hasAvatar ? '' : "bg-gradient-to-br $color" }}
                                ...
                                {{-- Infos --}}
                                <div class="min-w-0">
                                    <p class="font-medium text-gray-900 dark:text-white text-sm truncate max-w-[160px] lg:max-w-xs">
                                        {{ $user->name }}
                                    </p>
                                    <p class="text-xs text-gray-400 dark:text-gray-500 truncate max-w-[160px] lg:max-w-xs">
                                        {{ $user->email }}
                                    </p>
                                </div>
                            </div>
                        ...
                        {{-- Statut — 3 badges compacts en colonne --}}
                        <td class="px-4 py-3.5 w-48">
                            <div class="flex flex-col gap-1">
                                ...
                                @endif
                                {{-- Email vérifié --}}
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium w-fit
                        ...
                        <td class="px-4 py-3.5 w-36">
                            <div class="flex flex-wrap gap-1">
                                ...
                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300">
                                    {{ $role->name }}
                                </span>
                                @empty
                        ...
                        <td class="hidden xl:table-cell px-4 py-3.5 w-48">
                            <div class="flex flex-wrap gap-1">
                                ...
                                <span class="inline-flex px-1.5 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 max-w-[90px] truncate" title="{{ $permission->name }}">
                                    {{ $permission->name }}
                                </span>
                                @endforeach
                                @if($permCount > 2)
                                <span class="inline-flex px-1.5 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">
                                    +{{ $permCount - 2 }}
                                </span>
                                @elseif($permCount === 0)
                                <span class="text-xs text-gray-400">—</span>
                        ...
                        {{-- Date --}}
                        <td class="px-4 py-3.5 w-24 text-xs text-gray-400 whitespace-nowrap">
                        ...
                        <td class="px-4 py-3.5 w-28 text-right">
                            <div class="inline-flex items-center justify-end gap-1.5">
                                ...
                                <button type="button"
                                        ...
                                        class="open-toggle-modal inline-flex items-center justify-center w-8 h-8 rounded-lg transition
                                            {{ $user->status === 'actif'
                    ...
                    <tr>
                        <td colspan="7" class="px-5 py-16 text-center">
                            <div class="flex flex-col items-center gap-3 text-gray-400">
                                ...
                                <a href="{{ route('admin.users.index') }}" class="text-xs text-sky-600 hover:underline">Réinitialiser les filtres</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="px-6 py-5">
            ...
            <div class="flex gap-3">
                <button type="button" id="modalCancelBtn"
                        class="flex-1 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition">
                    Annuler
                </button>
                <button type="button" id="modalConfirmBtn"
                        class="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-amber-600 rounded-xl hover:bg-amber-700 transition">
                    Confirmer
                </button>
            </div>
        </div>
