 ...
 <div class="max-w-2xl mx-auto">
    <div class="mb-8 ml-4">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Modifier le jour férié</h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1">{{ $holiday->label }} — {{ $holiday->date->locale('fr')->isoFormat('dddd D MMMM YYYY') }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <form method="POST" action="{{ route('admin.holidays.update', $holiday) }}">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="label" name="label" value="{{ old('label', $holiday->label) }}" required
                        class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
                    @error('label')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">{{ old('description', $holiday->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
