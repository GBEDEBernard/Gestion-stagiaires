<x-app-layout>
    ...
    <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-5">
        <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-1 text-2xl font-semibold text-slate-900 dark:text-white">
                {{ ($stagesTrash->count() ?? 0) + ($etudiantsTrash->count() ?? 0) + ($personnelsTrash->count() ?? 0) + ($badgesTrash->count() ?? 0) }}
            </p>
        </div>
        ...
        <div class="rounded-xl border border-indigo-100 bg-indigo-50 px-4 py-3 dark:border-indigo-900 dark:bg-indigo-950">
            <p class="text-xs font-medium uppercase tracking-wider text-indigo-500 dark:text-indigo-400">Badges</p>
            <p class="mt-1 text-2xl font-semibold text-indigo-700 dark:text-indigo-300">{{ $badgesTrash->count() ?? 0 }}</p>
        </div>
    </div>
    ...
    <div class="space-y-3" x-data="{
        ...
        @if(($stagesTrash->count() ?? 0) === 0 && ($etudiantsTrash->count() ?? 0) === 0 && ($personnelsTrash->count() ?? 0) === 0 && ($badgesTrash->count() ?? 0) === 0)
