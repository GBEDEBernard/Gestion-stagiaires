<x-app-layout title="Statistiques de Présence - Admin">
    ...
    <style>
        ...
        /* ── BASE ── */
        .pres-wrap * { box-sizing: border-box; }
        .pres-wrap { font-family: var(--font); color: var(--text); }

        /* ── HEADER ── */
        .pres-header { display:flex; justify-content:space-between; align-items:flex-end; gap:1rem; flex-wrap:wrap; }
        .pres-header-badge {
            display:inline-block; font-size:.7rem; font-weight:600; letter-spacing:.12em; text-transform:uppercase;
            color:var(--emerald); background:rgba(16,185,129,.12); border:1px solid rgba(16,185,129,.25);
            border-radius:999px; padding:.25rem .75rem; margin-bottom:.5rem;
        }
        .pres-header-title { font-size:1.75rem; font-weight:700; color:#fff; margin:0; line-height:1.2; }
        .pres-header-sub { font-size:.9rem; color:var(--muted); margin:.25rem 0 0; }
        .pres-header-actions { display:flex; align-items:center; gap:.75rem; }
        .pres-live-dot {
            display:flex; align-items:center; gap:.45rem; font-size:.8rem; font-weight:500;
            color:var(--emerald); background:rgba(16,185,129,.1); border:1px solid rgba(16,185,129,.2);
        ...
        .pres-btn-export {
            ...
            background:var(--emerald); color:#fff; font-size:.85rem; font-weight:600;
            border-radius:.6rem; text-decoration:none; transition:all .2s;
        }
        .pres-btn-export:hover { background:var(--emerald-d); transform:translateY(-1px); }
        ...
        .pres-tab {
            padding:.45rem 1.1rem; border-radius:.5rem; font-size:.83rem; font-weight:500;
            color:var(--muted); text-decoration:none; transition:all .2s;
        }
        .pres-tab:hover { color:var(--text); background:var(--bg3); }
        .pres-tab.active {
            background:var(--bg3); color:#fff; font-weight:600;
            box-shadow:0 1px 3px rgba(0,0,0,.4);
        }
        .pres-tab-indicator { display:inline-block; width:6px; height:6px; background:var(--emerald); border-radius:50%; margin-left:.4rem; vertical-align:middle; }

        /* ── KPI GRID ── */
        .pres-kpis { display:grid; grid-template-columns:repeat(4,1fr); gap:1rem; }
        ...
        .pres-kpi {
            ...
            border-radius:1rem; padding:1.4rem 1.5rem; position:relative; overflow:hidden;
            transition:border-color .2s, transform .2s;
        }
        .pres-kpi:hover { border-color:var(--border-hi); transform:translateY(-2px); }
        .pres-kpi-accent {
            position:absolute; top:0; left:0; right:0; height:2px; border-radius:1rem 1rem 0 0;
        }
        ...
        .pres-kpi-value { font-size:2rem; font-weight:700; color:#fff; line-height:1; margin-bottom:.3rem; font-family:var(--mono); }
        .pres-kpi-label { font-size:.82rem; color:var(--muted); }
        .pres-kpi-sub { font-size:.75rem; color:var(--muted); margin-top:.5rem; padding-top:.5rem; border-top:1px solid var(--border); }
        ...
        /* ── SECTION TITLE ── */
        .pres-section-title {
            font-size:1rem; font-weight:600; color:#fff; margin:0 0 1rem;
            display:flex; align-items:center; gap:.6rem;
        }
        .pres-section-title::before {
            content:''; display:inline-block; width:3px; height:16px; border-radius:99px; background:var(--emerald);
        }

        /* ── CHARTS AREA ── */
        .pres-charts-grid { display:grid; grid-template-columns:1.6fr 1fr; gap:1rem; }
        @media(max-width:900px){ .pres-charts-grid{ grid-template-columns:1fr; } }

        .pres-charts-triple { display:grid; grid-template-columns:repeat(3,1fr); gap:1rem; }
        @media(max-width:900px){ .pres-charts-triple{ grid-template-columns:1fr; } }

        .pres-card {
            background:var(--bg2); border:1px solid var(--border);
            border-radius:1rem; padding:1.5rem; transition:border-color .2s;
        }
        ...
        /* ── QUICK ACTIONS ── */
        .pres-actions-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:1rem; }
        @media(max-width:700px){ .pres-actions-grid{ grid-template-columns:1fr; } }

        .pres-action-card {
            display:flex; flex-direction:column; align-items:center; justify-content:center;
            gap:.6rem; padding:1.4rem 1rem; border-radius:1rem; text-decoration:none;
            font-weight:600; font-size:.9rem; text-align:center; transition:all .25s;
        ...
        .act-rose { background:linear-gradient(135deg,rgba(244,63,94,.18),rgba(244,63,94,.08)); border-color:rgba(244,63,94,.25); color:var(--rose); }
        .act-rose::before { background:var(--rose); }
        .act-emerald { background:linear-gradient(135deg,rgba(16,185,129,.18),rgba(16,185,129,.08)); border-color:rgba(16,185,129,.25); color:var(--emerald); }
        .act-emerald::before { background:var(--emerald); }
        .act-blue { background:linear-gradient(135deg,rgba(59,130,246,.18),rgba(59,130,246,.08)); border-color:rgba(59,130,246,.25); color:var(--blue); }
        .act-blue::before { background:var(--blue); }

        /* ── GROUPS ── */
        .pres-groups-grid { display:grid; grid-template-columns:1fr 1fr; gap:1rem; }
        @media(max-width:600px){ .pres-groups-grid{ grid-template-columns:1fr; } }

        .pres-group-card { background:var(--bg2); border:1px solid var(--border); border-radius:1rem; padding:1.4rem; }
        .pres-group-head { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:1rem; }
        .pres-group-label { font-size:.95rem; font-weight:600; color:#fff; }
        .pres-group-role { font-size:.75rem; color:var(--muted); margin-top:.15rem; }
        .pres-group-count { font-size:1.6rem; font-weight:700; color:#fff; font-family:var(--mono); }
        .pres-group-denom { font-size:.9rem; color:var(--muted); font-family:var(--mono); }
        .pres-group-avg { font-size:.78rem; color:var(--muted); margin-bottom:.65rem; }
        .pres-progress-bg { height:6px; background:rgba(255,255,255,.07); border-radius:99px; overflow:hidden; }
        .pres-progress-fill { height:100%; border-radius:99px; transition:width .8s cubic-bezier(.34,1.56,.64,1); }

        /* ── TABLES ── */
        .pres-table-card { background:var(--bg2); border:1px solid var(--border); border-radius:1rem; overflow:hidden; }
        .pres-table-head { padding:1.2rem 1.5rem; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center; }
        .pres-table-head-title { font-size:.92rem; font-weight:600; color:#fff; }
        .pres-table-head-meta { font-size:.75rem; color:var(--muted); }
        table.pres-table { width:100%; border-collapse:collapse; }
        table.pres-table th {
            padding:.65rem 1.2rem; text-align:left; font-size:.72rem; font-weight:600;
            letter-spacing:.08em; text-transform:uppercase; color:var(--muted);
            background:rgba(255,255,255,.02); border-bottom:1px solid var(--border);
        }
        table.pres-table td { padding:.85rem 1.2rem; font-size:.85rem; color:var(--text); border-bottom:1px solid rgba(255,255,255,.04); }
        table.pres-table tbody tr:last-child td { border-bottom:none; }
        table.pres-table tbody tr:hover td { background:rgba(255,255,255,.02); }
        .pres-rank { font-size:.75rem; font-weight:700; font-family:var(--mono); color:var(--muted); }
        ...
        .tag-emerald { background:rgba(16,185,129,.12); color:var(--emerald); border:1px solid rgba(16,185,129,.2); }

        .pres-empty { text-align:center; padding:3rem 1rem; color:var(--muted); font-size:.85rem; }
        .pres-empty-icon { font-size:2rem; margin-bottom:.5rem; }

        /* ── REPORTS SECTION ── */
        .pres-reports-kpis { display:grid; grid-template-columns:repeat(4,1fr); gap:1rem; margin-bottom:1.25rem; }
        @media(max-width:900px){ .pres-reports-kpis{ grid-template-columns:repeat(2,1fr); } }

        .pres-reports-bottom { display:grid; grid-template-columns:1fr 1fr; gap:1rem; }
        @media(max-width:700px){ .pres-reports-bottom{ grid-template-columns:1fr; } }

        .pres-tips { background:rgba(16,185,129,.06); border:1px solid rgba(16,185,129,.15); border-radius:.85rem; padding:1.2rem 1.4rem; }
        .pres-tips-title { font-size:.87rem; font-weight:700; color:var(--emerald); margin-bottom:.6rem; }
        .pres-tips ul { margin:0; padding:0; list-style:none; display:flex; flex-direction:column; gap:.4rem; }
        .pres-tips li { font-size:.8rem; color:#6ee7b7; display:flex; align-items:flex-start; gap:.5rem; }
        .pres-tips li::before { content:'›'; color:var(--emerald); font-weight:700; flex-shrink:0; }
    ...
    <div class="pres-page">
        ...
        <div class="pres-kpis mt-6">
            ...
            <div class="pres-kpi kpi-emerald">
                ...
                <div class="pres-kpi-top">
                    <div class="pres-kpi-icon">
                        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/><path d="M9 16l2 2 4-4"/></svg>
                    </div>
            ...
            <div class="pres-kpi kpi-blue">
                ...
                <div class="pres-kpi-top">
                    <div class="pres-kpi-icon">
                        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 3h-8a2 2 0 00-2 2v2h12V5a2 2 0 00-2-2z"/></svg>
                    </div>
        ...
        <div class="mt-6">
            ...
            <div class="pres-charts-triple">

                {{-- Chart 1 : Présence --}}
                <div class="pres-card">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
                        <div>
                            <div style="font-size:.82rem;font-weight:600;color:#fff;">Présence</div>
                            <div style="font-size:.72rem;color:var(--muted);margin-top:.1rem;">Jours présents</div>
                        </div>
                        <div style="width:8px;height:8px;background:var(--emerald);border-radius:50%;"></div>
                    </div>
                ...
                <div class="pres-card">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
                        <div>
                            <div style="font-size:.82rem;font-weight:600;color:#fff;">À l'heure</div>
                            <div style="font-size:.72rem;color:var(--muted);margin-top:.1rem;">Arrivées ponctuelles</div>
                        </div>
                ...
                <div class="pres-card">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
                        <div>
                            <div style="font-size:.82rem;font-weight:600;color:#fff;">Retards</div>
                            <div style="font-size:.72rem;color:var(--muted);margin-top:.1rem;">Minutes cumulées</div>
                        </div>
        ...
        <div class="mt-6">
            ...
            <div class="pres-reports-kpis">
                ...
                <div class="pres-kpi kpi-amber">
                    <div class="pres-kpi-accent"></div>
                    <div class="pres-kpi-top"><div class="pres-kpi-icon">⏳</div></div>
        ...
        <div class="pres-charts-grid mt-6">
            ...
            <div class="pres-table-card">
                ...
                <table class="pres-table">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="4"><div class="pres-empty"><div class="pres-empty-icon">✅</div>Aucun retard détecté</div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="pres-table-card">
                ...
                @if(empty($absences))
                    <div class="pres-empty"><div class="pres-empty-icon">🎉</div>Aucune absence détectée</div>
                @else
                    <table class="pres-table">
                        <thead><tr><th>Utilisateur</th><th>Jours</th></tr></thead>
                        <tbody>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const late    = @json($globalStats['chart_data']['late'] ?? []);
        const onTime  = present.map((v, i) => Math.max(0, v - (late[i] ?? 0)));
        ...
        const baseOpts = (yLabel) => ({
            ...
            scales: {
                ...
                y: {
                    grid: gridOpts,
                    ticks: { font:{ size:11 }, callback: v => yLabel ? v + yLabel : v },
                    border: { display: false },
                    beginAtZero: true,
                }
            }
        ...
        /* ── Chart 1 : Présence ── */
        const ctxP = document.getElementById('chartPresence').getContext('2d');
        new Chart(ctxP, {
            type: 'line',
            data: {
                labels,
                datasets: [{
                    ...
                    borderColor: '#10b981',
                    borderWidth: 2.5,
                    backgroundColor: grad(ctxP, 'rgba(16,185,129,.25)', 'rgba(16,185,129,0)'),
                    fill: true,
                    tension: 0.45,
        ...
        /* ── Chart 2 : À l'heure ── */
        const ctxO = document.getElementById('chartOnTime').getContext('2d');
        new Chart(ctxO, {
            type: 'line',
            data: {
                labels,
                datasets: [{
                    ...
                    borderColor: '#3b82f6',
                    borderWidth: 2.5,
                    backgroundColor: grad(ctxO, 'rgba(59,130,246,.25)', 'rgba(59,130,246,0)'),
                    fill: true,
                    tension: 0.45,
        ...
        /* ── Chart 3 : Retards ── */
        const ctxL = document.getElementById('chartLate').getContext('2d');
        new Chart(ctxL, {
            type: 'bar',
            data: {
                labels,
        ...
        /* ── Chart Overview (grouped) ── */
        const ctxOv = document.getElementById('chartOverview').getContext('2d');
        new Chart(ctxOv, {
            type: 'bar',
            data: {
                labels,
