    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-0">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('employes.index') }}" class="p-2 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition shadow-sm" title="Retour à la liste">
                    <svg class="w-5 h-5 text-gray-600 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </a>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Modifier l'employé</h1>
            </div>
            <p class="text-gray-500 dark:text-gray-300 ml-11">Mettez à jour les informations de l'employé.</p>
        </div>

        <!-- Carte formulaire -->
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <form action="{{ route('employes.update', $employe) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label for="date_naissance" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Date de naissance</label>
                        <input type="date" name="date_naissance" id="date_naissance" value="{{ old('date_naissance', $p->date_naissance) }}"
                               class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl
                                      text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500 focus:border-transparent">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label for="domaine_id" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Domaine <span class="text-red-500">*</span></label>
                        <select name="domaine_id" id="domaine_id" required
                                class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl
                                       text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500 focus:border-transparent">
                            @foreach($domaines as $domaine)
                                <option value="{{ $domaine->id }}" {{ old('domaine_id', $employe->domaine_id) == $domaine->id ? 'selected' : '' }}>
                                    {{ $domaine->nom }}
                                </option>
                            @endforeach
                        </select>
                        @error('domaine_id') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="site_id" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Site <span class="text-red-500">*</span></label>
                        <select name="site_id" id="site_id" required
                                class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl
                                       text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500 focus:border-transparent">
                            @foreach($sites as $site)
                                <option value="{{ $site->id }}" {{ old('site_id', $employe->site_id) == $site->id ? 'selected' : '' }}>
                                    {{ $site->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('site_id') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label for="poste" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Poste</label>
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                    <a href="{{ route('employes.index') }}" class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition shadow-sm">
                        Annuler
                    </a>
