<x-app-layout>
    <div class="mb-8 ml-4">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Fiche employé</h1>
                <p class="text-gray-500 dark:text-gray-300 mt-1">Informations professionnelles et personnelles.</p>
            </div>
            <div class="flex gap-3 flex-wrap">
                <a href="{{ route('employes.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                    Retour
                </a>
                <a href="{{ route('employes.edit', $employe) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 rounded-xl hover:bg-yellow-200 dark:hover:bg-yellow-800/50 transition shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="px-6 pb-6 relative">
                    ...
                    <div class="text-center mb-6">
                        ...
                        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold mt-2 bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300">
                            Employé
                        </span>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 lg:col-span-2">
                ...
                <div class="mt-6 grid gap-6 md:grid-cols-2">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-4">
                        ...
                        <p class="text-gray-900 dark:text-white font-medium">{{ $employe->domaine->nom ?? '-' }}</p>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-4">
                        <div class="flex items-center gap-2 text-gray-500 dark:text-gray-400 text-sm mb-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <p class="text-gray-900 dark:text-white font-medium">{{ $employe->site->name ?? '-' }}</p>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-4">
                        <div class="flex items-center gap-2 text-gray-500 dark:text-gray-400 text-sm mb-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if($p->user)
                    <div class="mt-6 bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-4">
                        ...
                        <div class="grid gap-3 sm:grid-cols-3 text-sm">
                            <div>
                                ...
                                <span class="ml-1 text-gray-900 dark:text-white font-medium">{{ $p->user->status }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 dark:text-gray-400">Email vérifié :</span>
                                <span class="ml-1 text-gray-900 dark:text-white font-medium">{{ $p->user->email_verified_at ? 'Oui' : 'Non' }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 dark:text-gray-400">Doit changer mot de passe :</span>
                                <span class="ml-1 text-gray-900 dark:text-white font-medium">{{ $p->user->must_change_password ? 'Oui' : 'Non' }}</span>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="mt-8 flex flex-wrap gap-3 border-t border-gray-100 dark:border-gray-700 pt-6">
                    @if(!$p->user)
                        <form action="{{ route('employes.generate-account', $employe) }}" method="POST" class="inline">
                            @csrf
                    ...
                    @endif
                    <form action="{{ route('employes.destroy', $employe) }}" method="POST" onsubmit="return confirm('Supprimer définitivement cet employé ?');" class="inline">
                        @csrf
                        @method('DELETE')
