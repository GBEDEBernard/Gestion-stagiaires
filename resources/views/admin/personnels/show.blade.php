<x-app-layout>
    @php
        $user = $personnel->user; // Compte utilisateur lié (peut être null)
        $profil = $personnel->personnable; // Etudiant ou Employe (polymorphique)
        ...
        $nom = $personnel->nom ?? '';
        $initials = mb_strtoupper(mb_substr($prenom, 0, 1) . mb_substr($nom, 0, 1)) ?: 'PE';
        ...
        // Si stagiaire, récupérer l'historique des stages
        ...
        // Projets / Tâches de l'utilisateur
        ...
        } elseif ($profil instanceof \App\Models\Etudiant) {
            $tasksQuery->where('etudiant_id', $profil->id);
        } else {
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    <a href="{{ route('personnels.index') }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition">Personnels</a>
        ...
        <div class="flex flex-wrap gap-2.5">
            <a href="{{ encrypted_route('admin.personnels.badge.show', $personnel) }}"
            ...
            @if(!$user)
            <form action="{{ encrypted_route('personnels.generate-account', $personnel) }}" method="POST" class="inline">
                @csrf
                <button type="submit"
    ...
    @endif
    @if(session('error'))
        <div class="bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-xl mb-6 shadow-sm flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {{-- Left Column - Profile Card & Main Details --}}
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-2">
                                ...
                                @if($user)
                                    ...
                                    @endif
                                @else
                                    <span class="inline-flex items-center text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 px-2.5 py-1 rounded-lg">
                                        Sans compte
                                    </span>
                                @endif
                            </div>
        ...
        {{-- Right Column - Stats, Account & Actions --}}
        <div class="space-y-6">
            {{-- Statistics Card --}}
            <div class="bg-gradient-to-br from-violet-600 to-indigo-600 rounded-2xl shadow-xl overflow-hidden text-white">
                <div class="p-4 sm:p-5">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        @else
                        <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 min-w-0">
                            <p class="text-xl sm:text-2xl font-bold tracking-tight truncate">{{ $user ? '1' : '0' }}</p>
                            <p class="text-xs text-white/80 truncate">Compte actif</p>
                        </div>
                        <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 min-w-0">
                            <p class="text-base sm:text-lg font-bold tracking-tight truncate" title="{{ $personnel->created_at ? $personnel->created_at->format('d/m/Y') : '—' }}">
                                {{ $personnel->created_at ? $personnel->created_at->format('d/m/Y') : '—' }}
                            </p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4 text-sm">
                    ...
                    <div class="flex justify-between items-center"><span class="text-gray-500 dark:text-gray-400">Mise à jour</span><span class="font-medium text-gray-800 dark:text-white">{{ $user->updated_at->format('d/m/Y') }}</span></div>
                    ...
                    <div class="flex justify-between items-center"><span class="text-gray-500 dark:text-gray-400">Mot de passe</span><span class="font-medium text-gray-800 dark:text-white">{{ $user->must_change_password ? 'Temporaire' : 'Permanent' }}</span></div>
                    ...
                    <div class="flex justify-between items-center"><span class="text-gray-500 dark:text-gray-400">Superviseur</span><span class="font-medium text-gray-800 dark:text-white">{{ $supervisorName }}</span></div>
                </div>
            </div>
            @else
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-center text-gray-500 dark:text-gray-400 mb-4 text-sm">Aucun compte utilisateur associé à ce personnel.</p>
                    <form action="{{ encrypted_route('personnels.generate-account', $personnel) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-xl hover:from-sky-600 hover:to-blue-700 transition shadow-md text-sm font-medium">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 text-center text-sm text-gray-500 dark:text-gray-400">
                    <svg class="w-10 h-10 mx-auto mb-2 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 00-2 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002 2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                    </svg>
