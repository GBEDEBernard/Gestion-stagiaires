<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <form action="{{ encrypted_route('admin.roles.update', $role) }}" method="POST" class="p-6 space-y-6">
                @csrf @method('PUT')
    ...
    <script>
        document.querySelectorAll('.select-all-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                document.querySelectorAll(`.permission-checkbox[data-group="${group}"]`).forEach(cb => cb.checked = checked);
            });
        });
        ...
        function toggleAllPermissions() {
            ...
            allCheckboxes.forEach(cb => cb.checked = !anyChecked);
            document.querySelectorAll('.select-all-checkbox').forEach(checkbox => {
                const group = checkbox.dataset.group;
                const groupCheckboxes = document.querySelectorAll(`.permission-checkbox[data-group="${group}"]`);
                checkbox.checked = Array.from(groupCheckboxes).every(cb => cb.checked);
