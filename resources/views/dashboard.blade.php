<x-app-layout>
    ...
    {{-- ════════════════════════════════════════════════════════
         BANNER
    ════════════════════════════════════════════════════════ --}}
    <div class="relative ml-4 overflow-hidden bg-gradient-to-br from-slate-900 via-violet-900 to-slate-900 rounded-2xl mb-8">
    ...
    <div class="space-y-6 mt-4 relative z-10 ml-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="max-h-80 overflow-y-auto">
                ...
                <a href="{{ $notification->url }}"
                    ...
                    <form id="dash-notif-form-{{ $notification->id }}" action="{{ route('notifications.markRead', $notification->id) }}" method="GET" style="display: none;">
                    </form>
            ...
            <div class="px-6 py-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                <form action="{{ route('notifications.markAllRead') }}" method="POST" class="flex items-center justify-between">
                    @csrf
                    <button type="submit" class="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    @foreach([['Taux de réussite',$tauxReussite,'from-green-400 to-green-500','text-green-600'],['Étudiants actifs',$tauxEtudiantsActifs,'from-blue-400 to-blue-500','text-blue-600'],['Conversion',$tauxConversion,'from-violet-400 to-violet-500','text-violet-600']] as [$lbl,$val,$grad,$cls])
                    <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    @foreach([['Stages',$stagesTrash->count(),'text-purple-600'],['Étudiants',$etudiantsTrash->count(),'text-blue-600'],['Badges',$badgesTrash->count(),'text-amber-600'],['Services',$servicesTrash->count(),'text-rose-600']] as [$lbl,$cnt,$cls])
    ...
    {{-- ════════════════════════════════════════════════════════
         DONNÉES PHP → JSON (hors @push pour éviter l'erreur de parsing)
         On pré-encode toutes les variables ici, AVANT le @push
    ════════════════════════════════════════════════════════ --}}
    <script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            /* ── Données pré-encodées depuis window.__DASHBOARD__ ─── */
            var D = window.__DASHBOARD__;
            var LABELS_JOUR = D.labelsJour;
            var DATA_JOUR = D.evolutionJour;
            var LABELS_SEMAINE = D.labelsSemaine;
            var DATA_SEMAINE = D.evolutionSemaine;
            ...
            var STAGES_MOIS_LABELS = D.stagesMoisLabels;
            var STAGES_MOIS_DATA = D.stagesMoisData;
            ...
            window.switchPeriod = function(period) {
                var periods = ['jour', 'semaine', 'mois'];
                periods.forEach(function(p) {
                    var btn = document.getElementById('btn-' + p);
                    if (!btn) return;
                    if (p === period) {
                        btn.className = 'px-3 py-1.5 text-xs font-semibold rounded-md bg-blue-500 text-white transition-all';
                    } else {
                        btn.className = 'px-3 py-1.5 text-xs font-semibold rounded-md text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-all';
                    }
                });
                if (period === 'jour') {
                ...
                if (period === 'semaine') {
                    buildInscriptions(LABELS_SEMAINE, DATA_SEMAINE);
                }
                if (period === 'mois') {
                    buildInscriptions(LABELS_MOIS, DATA_MOIS);
                }
            };
            ...
            /* ══════════════════════════════════════════════════════
               2. PAR TYPE DE STAGE — Doughnut
            ══════════════════════════════════════════════════════ */
            (function() {
                var ctx = document.getElementById('chart-types');
                if (!ctx) return;

                var labels = TYPES_LABELS.length ? TYPES_LABELS : ['Aucun type'];
                var data = TYPES_DATA.length ? TYPES_DATA : [1];
            ...
            /* ══════════════════════════════════════════════════════
               3. STAGES CRÉÉS (12 derniers mois) — Barres
            ══════════════════════════════════════════════════════ */
            (function() {
                ...
                new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: STAGES_MOIS_LABELS,
                        datasets: [{
                            label: 'Stages créés',
                            data: STAGES_MOIS_DATA,
            ...
            (function() {
                ...
                var labels = SVC_LABELS.length ? SVC_LABELS : ['Aucun service'];
                var enCours = SVC_ENCOURS.length ? SVC_ENCOURS : [0];
                var termines = SVC_TERMINES.length ? SVC_TERMINES : [0];
                var inscrits = SVC_INSCRITS.length ? SVC_INSCRITS : [0];
