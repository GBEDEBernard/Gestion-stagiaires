{{-- 
    PAGE DE POINTAGE – ÉTUDIANTS (stagiaires)
    - Affiche le stage actif, le statut du jour
    - Récupère la position GPS avec une précision < 100 m (3 essais max)
    - Envoie vers prepareCheckin / prepareCheckout
    - Gère l'observation obligatoire en cas de retard (modale)
--}}
<x-app-layout>
    <main class="max-w-2xl mx-auto px-6 py-12 md:py-16">
        ...
        {{-- Vérification de l'existence d'un stage actif --}}
        @if(!$activeStage)
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // ─────────────────────────────────────────────────────────────────
            // 3. AFFICHAGE DES MESSAGES DE STATUT
            // ─────────────────────────────────────────────────────────────────
            ...
            // ─────────────────────────────────────────────────────────────────
            // 4. VARIABLES GLOBALES DE L'APPAREIL
            // ─────────────────────────────────────────────────────────────────
            ...
            // ─────────────────────────────────────────────────────────────────
            // 6. COLLECTE DE LA MEILLEURE POSITION (GPS + fallback IP)
            //    - 3 essais GPS maximum
            //    - Dès qu'une précision ≤ 100 m ou après 3 essais, on garde la meilleure
            //    - Fallback IP si GPS indisponible ou trop imprécis
            // ─────────────────────────────────────────────────────────────────
                ...
                if ('geolocation' in navigator) {
                    try {
                        ...
                        if (bestPos.coords.accuracy <= 50) {
                            ...
                            // Au-delà de 100 m, on marque comme faible
                            result.location_method = 'gps_low';
                            result.confidence_score = 30;
                        }
                    } catch (err) {
                        // GPS en échec (refus, timeout)
                        console.warn('GPS échoué :', err);
                    }
                }

                // --- Fallback par IP (si pas de position ou précision > 100 m) ---
                if (!result.latitude || result.accuracy_meters > 100) {
                    try {
                        ...
                        if (ipData.latitude && ipData.longitude) {
                            ...
                            result.accuracy_meters = 2000; // IP très imprécise
                            result.location_method = 'ip_fallback';
            ...
            /**
             * Tente d'obtenir la meilleure position GPS avec 3 essais max.
             * Résout dès qu'une position a une précision ≤ 100 m,
             * sinon renvoie la meilleure (celle avec la plus petite précision).
             */
            function getBestGPSPosition(onProgress) {
                return new Promise((resolve, reject) => {
                    ...
                    function tryOnce() {
                        if (onProgress) onProgress(`📍 Tentative GPS ${count+1}/${MAX_ATTEMPTS} (précision < 100m requise)`);
                        navigator.geolocation.getCurrentPosition(
                            (pos) => {
                                attempts.push(pos);
                                count++;
                                // Si précision déjà ≤ 100 m ou si on a épuisé les tentatives
                                if (pos.coords.accuracy <= 100 || count >= MAX_ATTEMPTS) {
                                    const best = attempts.reduce((a, b) => a.coords.accuracy < b.coords.accuracy ? a : b);
                                    resolve(best);
                                } else {
                                    setTimeout(tryOnce, 2500); // attendre 2.5s entre deux essais
                                }
                            },
                            (err) => {
                                if (attempts.length > 0) {
                                    // On a au moins une position → on la renvoie
                                    const best = attempts.reduce((a, b) => a.coords.accuracy < b.coords.accuracy ? a : b);
                                    resolve(best);
                                } else {
                                    reject(err);
            ...
            // ─────────────────────────────────────────────────────────────────
            // 7. REMPLISSAGE DES CHAMPS CACHÉS DU FORMULAIRE
            // ─────────────────────────────────────────────────────────────────
            function fillFormFields(form, posData) {
                // Déterminer le préfixe (checkin ou checkout) à partir de l'id du formulaire
                ...
                // Champs additionnels pour la méthode de localisation
            ...
            // ─────────────────────────────────────────────────────────────────
            // 8. GESTIONNAIRE DE CLIC POUR UN FORMULAIRE (ARRIVÉE OU DÉPART)
            // ─────────────────────────────────────────────────────────────────
            function handlePointing(formId, btnId) {
                ...
                btn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        if (!posData.latitude || !posData.longitude) {
                            throw new Error('Position introuvable');
                        }
                        ...
                        // Réactiver l'autre bouton s'il n'était pas désactivé à l'origine
