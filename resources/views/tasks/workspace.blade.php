<x-app-layout title="Tâches">
    ...
    <style>
        ...
        .ws-root {
            --ws-bg: #f7f7f6;
            --ws-text: #0a0a0a;
            --ws-text-soft:    rgba(0,0,0,.55);
            --ws-text-muted:   rgba(0,0,0,.50);
            --ws-text-faint:   rgba(0,0,0,.45);
            --ws-text-fainter: rgba(0,0,0,.38);
            --ws-text-dim:     rgba(0,0,0,.30);
            --ws-text-tag:     rgba(0,0,0,.70);
            --ws-border:       rgba(0,0,0,.06);
            --ws-border2:      rgba(0,0,0,.09);
            --ws-icon:         rgba(0,0,0,.35);
            --ws-icon-dim:     rgba(0,0,0,.30);
            --ws-active-text:  #fff;
            --ws-active-bg:    #0a0a0a;
            --ws-modal-bg:     #fff;
            --ws-toast-bg:     #0a0a0a;
        ...
        /* ── Dark mode tokens — contraste rehaussé ── */
        .dark .ws-root, .ws-root.dark {
            --ws-bg: #0f0f0f;
            --ws-text: #fff;
            --ws-text-soft:    rgba(255,255,255,.88);   /* était .55 */
            --ws-text-muted:   rgba(255,255,255,.78);   /* était .45 */
            --ws-text-faint:   rgba(255,255,255,.68);   /* était .40 */
            --ws-text-fainter: rgba(255,255,255,.58);   /* était .35 */
            --ws-text-dim:     rgba(255,255,255,.48);   /* était .30 */
            --ws-text-tag:     rgba(255,255,255,.90);
            --ws-border:       rgba(255,255,255,.08);
            --ws-border2:      rgba(255,255,255,.12);
            --ws-icon:         rgba(255,255,255,.55);
            --ws-icon-dim:     rgba(255,255,255,.45);
            --ws-active-text:  #0a0a0a;
            --ws-active-bg:    #fff;
            --ws-modal-bg:     #18181b;
            --ws-toast-bg:     #fff;
            --ws-toast-text:   #0a0a0a;
        }
        ...
        .ws-task-item.ws-active,
        .ws-task-item.ws-active .ws-task-title,
        .ws-task-item.ws-active .ws-task-pct,
        .ws-task-item.ws-active .ws-task-owner,
        .ws-task-item.ws-active .ws-task-date,
        .ws-task-item.ws-active .ws-progress-track,
        .ws-task-item.ws-active .ws-progress-bar {
            color: var(--ws-active-text);
        }
        .ws-task-title  { color: var(--ws-text); }
        .ws-task-pct    { color: var(--ws-text-faint); }
        .ws-task-owner  { color: var(--ws-text-faint); }
        .ws-task-date   { color: var(--ws-text-faint); }
        .ws-progress-track { background: rgba(0,0,0,.07); }
        .dark .ws-progress-track { background: rgba(255,255,255,.1); }
        ...
        .ws-input::placeholder { color: rgba(0,0,0,.35); }
        .ws-input:focus {
            background: #fff;
        ...
        /* ── Inputs dark — texte bien visible ── */
        .dark .ws-input,
        .ws-root.dark .ws-input {
            background: rgba(255,255,255,.07);
            border-color: rgba(255,255,255,.14);
            color: #ffffff;
        }
        .dark .ws-input::placeholder,
        .ws-root.dark .ws-input::placeholder {
            color: rgba(255,255,255,.40);
        }
        .dark .ws-input:focus,
        .ws-root.dark .ws-input:focus {
            background: rgba(255,255,255,.11);
            border-color: rgba(255,255,255,.30);
            box-shadow: 0 0 0 3px rgba(255,255,255,.07);
            color: #ffffff;
        }
        /* select options lisibles en dark */
        .dark .ws-input option,
        .ws-root.dark .ws-input option {
            background: #27272a;
            color: #ffffff;
        }
        ...
        .dark .ws-filter-pill { color: rgba(255,255,255,.65); }
        .dark .ws-filter-pill:hover { background: rgba(255,255,255,.07); color: rgba(255,255,255,.95); }
        ...
        .ws-btn-ghost { background: transparent; }
        .ws-btn-ghost:hover { background: rgba(0,0,0,.05); }
        .dark .ws-btn-ghost:hover { background: rgba(255,255,255,.08); }

        .ws-btn-cancel {
            background: transparent;
            color: var(--ws-text-soft);
            border-color: var(--ws-text-dim);
        }
        .ws-btn-cancel:hover { background: rgba(0,0,0,.04); }
        .dark .ws-btn-cancel {
            color: rgba(255,255,255,.75);
            border-color: rgba(255,255,255,.25);
        }
        .dark .ws-btn-cancel:hover { background: rgba(255,255,255,.08); }
        ...
        /* ── Modal dark : labels lisibles ── */
        .ws-root.dark .ws-modal-label,
        .dark .ws-root .ws-modal-label {
            color: rgba(255,255,255,.80) !important;
        }
        .ws-root.dark .ws-modal-eyebrow,
        .dark .ws-root .ws-modal-eyebrow {
            color: rgba(255,255,255,.65) !important;
        }
        .ws-root.dark .ws-modal-title,
        .dark .ws-root .ws-modal-title {
    ...
    <div class="ws-root -m-3 sm:-m-4 md:-m-6 min-h-screen" style="background: var(--ws-bg);" x-data="{ openCreate: {{ (isset($errors) && $errors->any()) && $canCreate ? 'true' : 'false' }} }" x-init="$el.classList.toggle('dark', document.documentElement.classList.contains('dark') || localStorage.getItem('theme') === 'dark')">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-0.5">
                                    <span class="text-[10px] font-semibold uppercase tracking-[.16em]" style="color:var(--ws-text-muted);">Workspace</span>
                                ...
                                <p class="mt-0.5 text-sm" style="color:var(--ws-text-soft);">Tâches, rapports et échanges en un seul endroit.</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <div class="flex items-center gap-2">
                                <div class="ws-stat-card rounded-xl px-4 py-2.5 text-center min-w-[80px]">
                                    <p class="text-[10px] font-medium uppercase tracking-[.14em]" style="color:var(--ws-text-muted);">Actives</p>
                                    <p class="mt-0.5 text-2xl font-semibold ws-mono" style="letter-spacing:-.02em;">{{ $activeTasks }}</p>
                                </div>
                                <div class="ws-stat-card rounded-xl px-4 py-2.5 text-center min-w-[80px]">
                                    <p class="text-[10px] font-medium uppercase tracking-[.14em]" style="color:var(--ws-text-muted);">Terminées</p>
                                    <p class="mt-0.5 text-2xl font-semibold ws-mono" style="color:#059669; letter-spacing:-.02em;">{{ $stats['completed'] ?? 0 }}</p>
                                </div>
                                <div class="ws-stat-card rounded-xl px-4 py-2.5 text-center min-w-[80px]">
                                    <p class="text-[10px] font-medium uppercase tracking-[.14em]" style="color:var(--ws-text-muted);">Taux</p>
            ...
            <div class="grid gap-4 lg:grid-cols-[340px_minmax(0,1fr)] xl:grid-cols-[380px_minmax(0,1fr)]">
                ...
                <aside class="ws-appear-2 min-w-0">
                    <div class="ws-card sticky top-20 rounded-2xl overflow-hidden">
                        ...
                        {{-- Search --}}
                        <div class="p-3 border-b" style="border-color:var(--ws-border);">
                            <form method="GET" action="{{ route('tasks.index') }}">
                                @if($status)<input type="hidden" name="status" value="{{ $status }}">@endif
                                <label class="relative flex items-center">
                                    <svg class="pointer-events-none absolute left-3 h-3.5 w-3.5" style="color:var(--ws-icon);" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                        ...
                        <div class="ws-scroll overflow-y-auto" style="max-height: calc(100vh - 20rem); min-height: 320px;">
                               ...
                               class="ws-task-item {{ $active ? 'ws-active' : '' }} relative block rounded-xl mx-2 my-1 p-3 overflow-hidden">
                                ...
                                <div class="pl-2">
                                    <div class="flex items-start justify-between gap-2 mb-2">
                                        <p class="ws-task-title text-sm font-medium line-clamp-2 leading-5">{{ $t->title }}</p>
                                        <span class="ws-task-pct ws-mono shrink-0 text-[11px] font-medium">{{ $pct }}%</span>
                                    </div>

                                    <div class="ws-progress-track h-[2px] rounded-full overflow-hidden mb-2.5">
                                        <div class="h-full rounded-full transition-all duration-500" style="width:{{ $pct }}%; background:{{ $active ? 'var(--ws-active-text)' : $barColor }};"></div>
                                    </div>

                                    <div class="flex items-center justify-between gap-2">
                                        <span class="ws-task-owner text-[11px] truncate">{{ $t->owner?->name ?? '—' }}</span>
                                        @if($t->due_date)
                                        <span class="ws-task-date ws-mono text-[11px] shrink-0" @if($t->isOverdue() && !$active) style="color:#ef4444;" @endif>
                            ...
                            <div class="flex min-h-[280px] flex-col items-center justify-center px-6 text-center">
                                <div class="flex h-10 w-10 items-center justify-center rounded-xl mb-3" style="background:var(--ws-text-dim);">
                                    <svg class="h-5 w-5" style="color:var(--ws-icon-dim);" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6M8 4h8a2 2 0 0 1 2 2v14l-3-2-3 2-3-2-3 2V6a2 2 0 0 1 2-2Z"/>
                                    </svg>
                                </div>
                ...
                <main class="ws-appear-3 min-w-0">
                    ...
                    <div class="ws-card flex min-h-[560px] items-center justify-center rounded-2xl p-8 text-center"
                         style="border-style: dashed; border-color: var(--ws-border2); background: transparent; box-shadow: none;">
                        <div class="max-w-xs">
                            <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-xl mb-4" style="background:var(--ws-text);">
                                <svg class="h-5 w-5" style="color:var(--ws-bg);" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 24 24">
        ...
        <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3500)"
             class="fixed bottom-5 right-5 z-50 flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium"
             style="background:var(--ws-toast-bg); color:var(--ws-toast-text); box-shadow: 0 4px 24px rgba(0,0,0,.25); max-width: 340px;">
